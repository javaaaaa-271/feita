...
function inspectGif(bytes: Uint8Array): StructuralInspection {
  ...
  while (offset < bytes.byteLength) {
    ...
    if (marker === 0x3b) return { kind: "gif", animated: frames > 1, truncated: false };
    if (marker === 0x2c) {
...
function inspectPng(bytes: Uint8Array): StructuralInspection {
  ...
  while (offset + 12 <= bytes.byteLength) {
    ...
    if (end > bytes.byteLength) return { kind: "png", animated, truncated: true };
...
export function isAcceptedStaticRaster(inspection: StructuralInspection) {
  return !inspection.animated && !inspection.truncated &&
    (inspection.kind === "jpeg" || inspection.kind === "png" || inspection.kind === "webp");
}
...
export function hasWebPSignature(bytes: Uint8Array) {
  return bytes.byteLength >= 12 && ascii(bytes, 0, 4) === "RIFF" &&
...
export function resizeOptions(width: number, height: number) {
  ...
  return width >= height ? { width: OUTPUT_MAX_DIMENSION } : { height: OUTPUT_MAX_DIMENSION };
}
